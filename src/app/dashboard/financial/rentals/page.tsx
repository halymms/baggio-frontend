...
export default function RentalsPage() {
    ...
    const selectedSection = 1; // Fixed to 1 for Locação
    ...
    useEffect(() => {
        ...
        realtimeReportData(body)
            .then((res) => {
                ...
                if (totalDespesasPessoalExtrasCalc || despesasNormais || comissaoGestorCalc || fundoInovacao || comissaoFolha || investimentosVals || salarios) {
                    const calculoTotalDespesas = Math.abs(totalDespesasPessoalExtrasCalc ?? 0) + Math.abs(salarios ?? 0) + Math.abs(comissaoFolha ?? 0) + Math.abs(despesasNormais ?? 0) + Math.abs(fundoInovacao ?? 0) + Math.abs(investimentosVals ?? 0) + Math.abs(comissaoGestorCalc ?? 0);
                    setTotalDespesas(calculoTotalDespesas);
                }

                if (resultadoLiquidoCalc || calculoFolhaPagamento || investimentosVals || comissaoGestorCalc) {
                    const retiradaCalc = (resultadoLiquidoCalc ?? 0) - Math.abs(calculoFolhaPagamento ?? 0) - Math.abs(investimentosVals ?? 0) - Math.abs(comissaoGestorCalc ?? 0);
                    setRetirada(retiradaCalc);

                    if (receita?.amount) {
                        const porcentagemRetiradaCalc = Math.abs(retiradaCalc) / Math.abs(receita.amount) * 100;
    ...
    }, [queryMonth, queryYear, totalDespesasPessoal, totalDespesasPessoalExtras, fundoInovacao, impostos, lucroLiquido]);
    ...
    return (
        <div className={styles.financialPageContainer}>
            <div className={styles.financialPageHeader}>
                <h1 className={styles.financialPageTitle}>Financeiro - Locação</h1>
                <p className={styles.financialPageSubtitle}>Visão geral das receitas e despesas de Locação</p>
            </div>
